...
export default function CreateTransDetail() {
  ...
  const [bookingRequests, setBookingRequests] = useState([
    { rankID: "", typeID: "", floor: "", roomCount: 1, peopleCount: 1, fromDate: "", toDate: "" }
  ]);
  ...
  const roomRanks = Array.from(new Map(rooms.map(r => [r.RankID, { id: r.RankID, name: r.RankName }])).values());
  const roomTypes = Array.from(new Map(rooms.map(r => [r.TypeID, { id: r.TypeID, name: r.TypeName }])).values());
  const capacityMap = new Map(rooms.map(r => [r.TypeID, r.Capacity]));

  const addRequest = () => setBookingRequests([
    ...bookingRequests,
    { rankID: "", typeID: "", floor: "", roomCount: 1, peopleCount: 1, fromDate: "", toDate: "" }
  ]);

  const removeRequest = (index) => setBookingRequests(bookingRequests.filter((_, i) => i !== index));
  ...
  const handleSubmit = async (e) => {
    ...
    for (const r of bookingRequests) {
      ...
      if (peopleCountNum > maxCapacity) {
        const typeName = roomTypes.find(t => t.id === typeIDNum)?.name || "unknown";
        setMessage(
    ...
    <div className="create-trans-page">
      ...
      <form onSubmit={handleSubmit}>
          ...
          <div className="request-card" key={idx}>
            ...
            <div className="form-row multi">
              <div>
                ...
                <select value={req.rankID} onChange={e => handleChange(idx, "rankID", e.target.value)}>
                  <option value="">-- Chọn hạng phòng --</option>
                  {roomRanks.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}
                </select>
              </div>
              <div>
                <label>Loại phòng:</label>
                <select value={req.typeID} onChange={e => handleChange(idx, "typeID", e.target.value)}>
              ...
              <div>
                <label>Tầng</label>
                <select value={req.floor} onChange={e => handleChange(idx, "floor", e.target.value)}>
            ...
            <div className="form-row multi">
              <div>
                ...
                <input type="number" min={1} value={req.roomCount} onChange={e => handleChange(idx, "roomCount", e.target.value)} />
              </div>
              <div>
                <label>Số lượng thành viên:</label>
                <input type="number" min={1} value={req.peopleCount} onChange={e => handleChange(idx, "peopleCount", e.target.value)} />
              </div>
            </div>
            ...
            <div className="form-row multi">
              <div>
                <label>From:</label>
                <input type="date" value={req.fromDate} onChange={e => handleChange(idx, "fromDate", e.target.value)} />
              </div>
              <div>
                <label>To:</label>
                <input type="date" value={req.toDate} onChange={e => handleChange(idx, "toDate", e.target.value)} />
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="form-actions">
          <button type="button" className="btn-secondary" onClick={addRequest}>+ Add Request</button>
