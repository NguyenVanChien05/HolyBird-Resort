...
exports.guestBookRoom = async (req, res) => {
  ...
  // Lấy GroupID từ token của Guest
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .input("ClientPrice", sql.Decimal(18, 0), clientPrice) // Quan trọng
    ...
    // Kiểm tra nếu thông báo lỗi chứa từ khóa từ SQL Server gửi lên
    if (err.message.includes("Giá phòng đã thay đổi") || err.message.includes("Vui lòng tải lại")) {
      // Trả về đúng mã 409 Conflict
